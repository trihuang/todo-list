...
class Controller {
    ...
    handleToggleCheckCircle = (id, event) => {
        ...
        if (target.status !== 'Completed') {
            ...
            if (hasAllSubTasksCompleted) {
                ...
                if (target.isProject) {
                    modalBody.textContent = 'All todos and subtodos must be completed to mark a project as complete.';
                    const modal = new bootstrap.Modal(alert);
                    modal.show();
                } else {
                    modalBody.textContent = 'All subtodos must be completed to mark a todo as complete.';
                    const modal = new bootstrap.Modal(alert);
                    modal.show();
                }
            }
    ...
    handleEditProjectBtnEventListener() {
        this.view.bindEditProjectBtnEventListener(this.handleEditProjectModal);
    }

    handleEditProjectModal = (projectID) => {
        let project = this.model.findById(this.model.projects, projectID);
        project = project[0];
        this.view.showEditProjectModal(project);
    }

    handleDeleteBtnEventListener() {
